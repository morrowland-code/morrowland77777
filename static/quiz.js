...
if (quizDiv) {
  quizDiv.innerHTML = QUESTIONS.map(
    (item, i) => `
    <div class="q">
...
function scoreAll() {
  ...
  const levels = {
    openness: bucket(avgs.openness),
    conscientiousness: bucket(avgs.conscientiousness),
    extraversion: bucket(avgs.extraversion),
    agreeableness: bucket(avgs.agreeableness),
    neuroticism: bucket(avgs.neuroticism),
  };

  const code = [
    levels.openness,
    levels.conscientiousness,
    levels.extraversion,
    levels.agreeableness,
    levels.neuroticism,
  ].join("-");

  return { code, avgs, levels };
}
...
if (submitBtn) {
  submitBtn.addEventListener("click", async () => {
    try {
      const { code, avgs, levels } = scoreAll();

      console.log("Final Big 5 Code:", code);
      console.log("Averages:", avgs);
      console.log("Levels:", levels);

      // Save the code to session server-side
      const response = await fetch("/api/set-latest-code", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error("Failed to save your result.");
      }
